...
import transporter from "./src/apis/nodemailer.js"
import firebaseApp from "./src/apis/firebase.js"
import {getStorage, ref, uploadString, getDownloadURL} from "firebase/storage"
import Datastore from "nedb"

const freesellApp = express()
freesellApp.use(express.json({limit: "3mb"}))
freesellApp.use(cookieParser())
freesellApp.use(cors({origin: "*"})) 

const freesellDB = {
  users: new Datastore("./src/database/users.db"),
  products: new Datastore("./src/database/products.db")
}
...
freesellApp.post("/freesell/api/changepassword", (req, res) => {
  ...
  freesellDB.users.update({_id: user, recoveryCode: code}, {$set: {pass: pass}}, {multi: true}, (err, nreplaced) => {
